...
public class Limelight extends SubsystemBase {
    ...
    public Pose2d apriltagBasedPosition() {
        ...
        LimelightHelpers.PoseEstimate positionEstimate = LimelightHelpers
        ...
        if (limelightName == LimelightConstants.ROBOT_LIMELIGHT_NAME) {
           ...
           * else if (limelightName == LimelightConstants.TURRET_LIMELIGHT_NAME) {
           * spinningTooFastToBeUseful = (Math.abs(SHOOTER.getTurretVelocity() +
           * SWERVE.getSpinRate())) > LimelightConstants.MAX_VISION_SPIN;
           * }
           */
    ...
    public Command updateRobotPosition() {
        return Commands.runOnce(() -> {
            ...
            SWERVE.resetOdometry(
                    new Pose2d(pose.getX(), pose.getY(), new Rotation2d(Math.toRadians(SWERVE.gyro.getAngle()))));
        });
    }
}